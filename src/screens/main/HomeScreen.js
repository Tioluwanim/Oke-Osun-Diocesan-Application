...
export default function HomeScreen({ navigation }) {
  ...
  const { data: liveStream, isLoading: ll } = useQuery({
  ...
  const { data: sermons = [], isLoading: sl } = useQuery({
    queryKey: queryKeys.sermons,
    queryFn: queryFns.sermons,
  });
  const { data: parishes = [], isLoading: pl } = useQuery({
  ...
  const updates = useMemo(
    () => [
      ...events.slice(0, 2).map((e) => ({
        ...
        color: COLORS.gold,
        title: e.title,
        body: e.location || 'Diocesan event',
        date: formatDate(e.date),
      })),
      ...sermons.slice(0, 1).map((s) => ({
        id: `s-${s.id}`,
        tag: 'Sermon',
        color: COLORS.teal,
        title: s.title,
        body: s.preacher || 'Diocese clergy',
        ...
        <View style={styles.headerLeft}>
          ...
          <View>
            <Text style={styles.greeting}>
              {getGreeting()}, {user?.fullName?.split(' ')[0] || 'Beloved'}
            </Text>
        ...
        <Animated.View style={[styles.statsRow, stagger(0)]}>
          {stats.map((s) => (
            <View
              ...
              style={[
                styles.statCard,
                { flexBasis: cardBasis, maxWidth: cardBasis },
              ]}
            >
              <AppIcon
                name={s.icon}
        ...
        <Animated.View style={stagger(1)}>
          ...
          {isLoading ? (
            ...
            <FeatureCard
              ...
              meta={`${formatDate(nextEvent.date)}${
        ...
        <Animated.View style={stagger(3)}>
          ...
          <View style={styles.quickGrid}>
            {QUICK_ACTIONS.map((a) => (
              <TouchableOpacity
                key={a.label}
                style={[styles.quickCard, { backgroundColor: a.accent }]}
                onPress={() => nav(a.screen)}
                activeOpacity={0.8}
              >
        ...
        <Animated.View style={stagger(4)}>
          ...
          {isLoading ? (
              ...
              <View key={u.id} style={styles.updateCard}>
                ...
                <View style={styles.updateBody}>
                  <View style={styles.updateTop}>
                    <Text style={[styles.updateTag, { color: u.color }]}>
                      {u.tag}
                    </Text>
            ...
            <EmptyState
              ...
              description="Events and sermons will appear here."
            />
          )}
        </Animated.View>

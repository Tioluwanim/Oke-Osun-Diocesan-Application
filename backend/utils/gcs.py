...
def _get_storage_client() -> storage.Client:
    ...
    if not os.path.exists(creds_path):
        raise HTTPException(
            status_code=500,
            detail=f"GCP credentials file not found at: {creds_path}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to initialize Google Cloud Storage client: {exc}"
        )
...
def upload_file_to_gcs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to upload file to Google Cloud Storage: {exc}"
        )

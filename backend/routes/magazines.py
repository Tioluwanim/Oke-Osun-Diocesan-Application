...
from models.magazine import CreateMagazineRequest, UpdateMagazineRequest
...
@router.post("/")
async def create_magazine(request: CreateMagazineRequest, current_user: dict = Depends(require_clergy)):
    mag = request.model_dump()
    mag["title"]        = sanitize_string(mag["title"])
    mag["uploadedBy"]   = current_user["email"]
    mag["uploaderName"] = current_user["fullName"]
    mag["isNew"]        = True
    mag["createdAt"]    = utcnow()
    mag["updatedAt"]    = utcnow()
